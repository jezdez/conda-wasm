...
User: conda install numpy
        ...
        ▼
┌─────────────────────────────────────────────────┐
│ conda (Python, patched for Emscripten)          │
│                                                 │
│  1. Parse spec ("numpy")                        │
│  2. Fetch repodata via requests (browser fetch) │
│  3. Call conda-wasm solver plugin              │
│  4. Download packages via requests              │
│  5. Extract via conda-wasm runtime              │
│  6. Link to prefix (copy-only on MEMFS)         │
└─────────────────────────────────────────────────┘
        │           │             │
        ▼           ▼             ▼
   ┌─────────┐ ┌──────────┐ ┌──────────────┐
   │ Solver  │ │ requests │ │ conda-wasm   │
...
Two options:
1. **conda-wasm runtime**: Python calls JS which calls conda-wasm's streaming extract
   (already implemented in Rust/WASM, proven fast)
2. **Pure Python**: Use `tarfile` + `zipfile` (stdlib, slow but works)

Recommended: conda-wasm runtime for `.conda` and `.tar.bz2` formats.
...
- [x] patched conda package (noarch)
