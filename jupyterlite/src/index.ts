...
import { CondaWasmWebWorkerKernel } from './kernel';
...
/**
 * JupyterLite plugin that registers xeus kernels using CondaWasmWebWorkerKernel
 * instead of the default WebWorkerKernel. This rewrites %conda commands
 * to %conda_wasm so they bypass mambajs and reach real conda in Python.
 *
...
const plugin: JupyterFrontEndPlugin<void> = {
  id: '@conda-wasm/jupyterlite:register',
  autoStart: true,
  ...
  activate: async (
    ...
    try {
      ...
      console.log(`conda-wasm jupyterlite: could not fetch xeus/kernels.json: ${err}`);
      throw err;
    }
    ...
    for (const kernelItem of kernelList) {
      ...
      kernelspecs.register({
        ...
        create: async (options: IKernel.IOptions): Promise<IKernel> => {
          ...
          return new CondaWasmWebWorkerKernel({
            ...options,
            contentsManager,
            mountDrive,
    ...
    console.log(
      'conda-wasm jupyterlite: registered kernels with %%conda -> %%conda_wasm rewrite'
    );
  }
};
