...
Usage::
    ...
    # Local build: adds the project's output/ channel and conda-wasm
...
LOCAL_EXTRA_SPECS = ["conda-wasm", "conda"]
...
dependencies:
  ...
  - matplotlib
  - ipywidgets
  # conda-wasm runtime + solver plugin (enables conda install from kernel)
  - conda-wasm
  # patched conda
  - conda
"""
...
def create_prefix(channels: list[str], specs: list[str]) -> pathlib.Path:
...
def main() -> None:
    ...
    parser.add_argument(
        "--with-local",
        action="store_true",
        help="Include locally-built conda packages (conda-wasm) from output/",
    )
    ...
    if args.with_local:
        if not output.exists():
            ...
            print("  pixi run -e recipes build-conda-wasm-lockfile")
            sys.exit(1)
    ...
    prefix_path = create_prefix(channels, specs)

    jupyterlite_ext = ROOT / "jupyterlite"
    if (jupyterlite_ext / "package.json").exists():
        print("[build.py] Building conda-wasm JupyterLite extension ...")
        subprocess.run(["jlpm", "install", "--immutable"], cwd=jupyterlite_ext, check=True)
        subprocess.run(["jlpm", "run", "build"], cwd=jupyterlite_ext, check=True)
        print("[build.py] conda-wasm JupyterLite extension built")
