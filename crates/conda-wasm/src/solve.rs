...
#[wasm_bindgen]
pub fn conda_wasm_solve_init() -> String {
    web_sys::console::log_1(&"conda-wasm solver initialized".into());
    format!("conda-wasm solver v{}", env!("CARGO_PKG_VERSION"))
}
...
) -> Result<Vec<GenericVirtualPackage>, CondaWasmError> {
    vpkgs
        .iter()
        .map(|vp| {
            let name = PackageName::from_str(&vp.name).map_err(|e| {
                CondaWasmError::PackageParse(format!("virtual package name '{}': {e}", vp.name))
            })?;
            let version = match &vp.version {
                Some(v) => Version::from_str(v).map_err(|e| {
                    CondaWasmError::PackageParse(format!("virtual package version '{v}': {e}"))
                })?,
...
pub(crate) fn solve_with_records(
...
) -> Result<SolveSolution, CondaWasmError> {
    web_sys::console::log_1(
        &format!(
            "conda-wasm: solving {} specs against {} repodata sources ({} total records)",
            specs.len(),
            available.len(),
    ...
    let solved = resolvo::Solver
        .solve(solver_task)
        .map_err(|e| CondaWasmError::SolveFailed(format!("{e}")))?;

    web_sys::console::log_1(
        &format!("conda-wasm: solved — {} packages", solved.records.len()).into(),
    );
...
fn embedded_emscripten_version() -> &'static str {
    #[cfg(conda_wasm_embedded_emscripten_version)]
    {
        include_str!(concat!(env!("OUT_DIR"), "/embedded_emscripten_version.txt"))
    }
    #[cfg(not(conda_wasm_embedded_emscripten_version))]
    {
        "3.1.58"
    }
}
