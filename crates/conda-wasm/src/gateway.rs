...
//!
//! [`conda_wasm_fetch_and_solve`] fetches repodata for all channel/subdir pairs, feeds
...
#[wasm_bindgen]
pub fn conda_wasm_fetch_and_solve(
    request: JsValue,
    ...
    let request = if request.is_string() {
        ...
        js_sys::JSON::parse(&json_str)
            .map_err(|e| CondaWasmError::InvalidInput(format!("JSON parse: {e:?}")))?
    } else {
        request
    };

    let req: FetchAndSolveRequest = serde_wasm_bindgen::from_value(request).map_err(|e| {
        CondaWasmError::InvalidInput(format!("parsing fetch_and_solve request: {e}"))
    })?;

    if req.channels.is_empty() {
        return Err(CondaWasmError::InvalidInput("no channels provided".into()).into());
    }
    if req.specs.is_empty() {
        return Err(CondaWasmError::InvalidInput("no specs provided".into()).into());
    }
    ...
    let specs: Vec<MatchSpec> = req
        ...
        .map(|s| {
            MatchSpec::from_str(s, rattler_conda_types::ParseStrictness::Lenient)
                .map_err(|e| CondaWasmError::SpecParse(format!("'{s}': {e}")))
        })
        .collect::<Result<_, _>>()?;
            ...
            match fetch_repodata_records(&ch.url, sd, &req.seed_names, fetch_binary, fetch_text) {
                Ok(recs) => {
                    web_sys::console::log_1(
                        &format!(
                            "conda-wasm: fetched {} records for {}/{}",
                            recs.len(),
                            ch.url,
                            sd,
                        )
                ...
                Err(e) => {
                    web_sys::console::warn_1(
                        &format!(
                            "conda-wasm: failed to fetch repodata for {}/{}: {}",
                            ch.url, sd, e,
                        )
                        .into(),
                    );
    ...
    for pass in 0..MAX_CROSS_CHANNEL_PASSES {
        ...
        web_sys::console::log_1(
            &format!(
                "conda-wasm: cross-channel pass {}: resolving {} deps ({:?})",
                pass + 1,
                missing.len(),
            ...
            for sd in &ch.subdirs {
                ...
                match crate::sharded::fetch_sharded_records(
                    ...
                    Ok(recs) if !recs.is_empty() => {
                        web_sys::console::log_1(
                            &format!(
                                "conda-wasm: resolved {} cross-channel records from {}/{}",
                                recs.len(),
                                ch.url,
                                sd,
                            )
...
#[wasm_bindgen]
pub fn conda_wasm_get_shard_urls(
    request: &str,
    fetch_binary: &js_sys::Function,
) -> Result<JsValue, JsValue> {
    let req: GetShardUrlsRequest = serde_json::from_str(request).map_err(|e| {
        CondaWasmError::InvalidInput(format!("parsing get_shard_urls request: {e}"))
    })?;
        ...
        for subdir in &ch.subdirs {
            ...
            let index =
                match crate::sharded::get_or_fetch_index(&cache_key, base, subdir, fetch_binary) {
                    ...
                    Err(e) => {
                        web_sys::console::log_1(
                            &format!("conda-wasm: no shard index for {cache_key}, skipping ({e})")
                                .into(),
                        );
                        continue;
                    }
    ...
    web_sys::console::log_1(
        &format!(
            "conda-wasm: computed {} shard URLs for {} seeds across {} channels",
            urls.len(),
            req.seeds.len(),
            req.channels.len(),
        )
    ...
    records
        ...
        .map(|r| {
            ...
            let url = url::Url::parse(&url_str).map_err(|e| {
                CondaWasmError::PackageParse(format!("invalid URL for {}: {e}", r.name))
            })?;

            let name = PackageName::from_str(&r.name).map_err(|e| {
                CondaWasmError::PackageParse(format!("package name '{}': {e}", r.name))
            })?;
            let version = Version::from_str(&r.version).map_err(|e| {
                CondaWasmError::PackageParse(format!("version '{}': {e}", r.version))
            })?;
            ...
            Ok(rattler_conda_types::RepoDataRecord {
                ...
                identifier: fn_name.parse().map_err(|e| {
                    CondaWasmError::PackageParse(format!("identifier for {}: {e}", r.name))
                })?,
                url,
