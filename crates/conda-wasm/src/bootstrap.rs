...
pub(crate) fn get_records(
    ...
    platform: Platform,
) -> Result<Vec<RepoDataRecord>, CondaWasmError> {
    let reader = Cursor::new(lockfile_content.as_bytes());
    let lockfile =
        LockFile::from_reader(reader).map_err(|e| CondaWasmError::LockfileParse(e.to_string()))?;
    let env = lockfile
        .default_environment()
        .ok_or(CondaWasmError::NoDefaultEnvironment)?;

    env.conda_repodata_records(platform)
        .map_err(|e| CondaWasmError::LockfileParse(e.to_string()))?
        .ok_or_else(|| CondaWasmError::NoRecordsForPlatform(platform.as_str().to_string()))
}
...
) -> Result<StreamingBootstrapResult, CondaWasmError> {
    let platform = Platform::from_str(platform_str)
        .map_err(|_| CondaWasmError::PlatformUnknown(platform_str.to_string()))?;
...
async fn download_and_extract_package_streaming(
    ...
    on_file: &js_sys::Function,
) -> Result<extract::ExtractStats, CondaWasmError> {
    ...
    if url.ends_with(".conda") {
        ...
        Err(CondaWasmError::UnknownPackageFormat(url))
    }
}
