...
//!
//! Inspired by rattler's gateway cache, this module caches both shard indices and individual shard
//! contents in memory for the lifetime of the WASM instance.  This avoids
//! redundant network fetches when the user runs multiple `%conda_wasm install`
...
#[wasm_bindgen]
pub fn conda_wasm_clear_repodata_cache() {
    INDEX_CACHE.with(|c| c.borrow_mut().clear());
    SHARD_CACHE.with(|c| c.borrow_mut().clear());
    web_sys::console::log_1(&"conda-wasm: repodata cache cleared".into());
}
...
#[wasm_bindgen]
pub fn conda_wasm_decode_shard_deps(data: &[u8]) -> Result<JsValue, JsValue> {
    let shard =
    ...
    serde_wasm_bindgen::to_value(&names)
        .map_err(|e| -> JsValue { CondaWasmError::SerializeFailed(e.to_string()).into() })
}
...
fn decompress_zstd(compressed: &[u8]) -> Result<Vec<u8>, CondaWasmError> {
    if compressed.is_empty() {
        return Err(CondaWasmError::RepodataParse(
            "empty zstd input (server returned 0 bytes)".into(),
        ));
    }

    let mut decoder = ruzstd::decoding::StreamingDecoder::new(compressed)
        .map_err(|e| CondaWasmError::RepodataParse(format!("zstd init: {e}")))?;
    ...
    loop {
        let n = decoder
            .read(&mut buf)
            .map_err(|e| CondaWasmError::RepodataParse(format!("zstd decompress: {e}")))?;
        if n == 0 {
            break;
        }
        decompressed.extend_from_slice(&buf[..n]);
        if decompressed.len() > MAX_DECOMPRESSED_SIZE {
            return Err(CondaWasmError::RepodataParse(format!(
...
fn decode_shard_index(compressed: &[u8]) -> Result<DecodedShardIndex, CondaWasmError> {
    let decompressed = decompress_zstd(compressed)?;
    let index: RawShardedRepodata = rmp_serde::from_slice(&decompressed)
        .map_err(|e| CondaWasmError::RepodataParse(format!("msgpack decode shard index: {e}")))?;
...
fn decompress_and_parse_shard(data: &[u8], label: &str) -> Result<Shard, CondaWasmError> {
    let decompressed = decompress_zstd(data)?;
    rmp_serde::from_slice(&decompressed).map_err(|e| {
        CondaWasmError::RepodataParse(format!("msgpack decode shard for {label}: {e}"))
    })
}
...
fn call_fetch_binary(cb: &js_sys::Function, url: &str) -> Result<Vec<u8>, CondaWasmError> {
    let js_url = JsValue::from_str(url);
    let result = cb
        .call1(&JsValue::NULL, &js_url)
        .map_err(|e| CondaWasmError::FetchFailed(format!("{url}: {e:?}")))?;

    let array = js_sys::Uint8Array::new(&result);
    Ok(array.to_vec())
}

fn call_fetch_text(cb: &js_sys::Function, url: &str) -> Result<String, CondaWasmError> {
    let js_url = JsValue::from_str(url);
    let result = cb
        .call1(&JsValue::NULL, &js_url)
        .map_err(|e| CondaWasmError::FetchFailed(format!("{url}: {e:?}")))?;
    ...
    result
        .as_string()
        .ok_or_else(|| CondaWasmError::FetchFailed(format!("{url}: response was not a string")))
}
...
pub(crate) fn get_or_fetch_index(
...
) -> Result<Rc<DecodedShardIndex>, CondaWasmError> {
    let cached = INDEX_CACHE.with(|c| c.borrow().get(cache_key).cloned());
    if let Some(index) = cached {
        web_sys::console::log_1(
            &format!("conda-wasm: shard index cache hit for {cache_key}").into(),
        );
        return Ok(index);
    }
    ...
    web_sys::console::log_1(
        &format!(
            "conda-wasm: cached shard index for {cache_key} ({} packages)",
            index.shards.len()
        )
        .into(),
    );
...
fn get_or_fetch_shard(
    ...
    base_url: &str,
    channel_url: &str,
) -> Result<Rc<CachedShard>, CondaWasmError> {
    ...
    let shard_bytes = call_fetch_binary(fetch_binary, shard_url)
        .map_err(|e| CondaWasmError::FetchFailed(format!("shard {name}: {e}")))?;
    ...
    for (id, rec) in shard.packages.iter().chain(shard.conda_packages.iter()) {
        ...
        let url = url::Url::parse(&format!("{base_url}{id}"))
            .map_err(|e| CondaWasmError::RepodataParse(format!("invalid URL for {id}: {e}")))?;
...
) -> Result<Vec<rattler_conda_types::RepoDataRecord>, CondaWasmError> {
        ...
        match fetch_sharded_records(base, subdir, seeds, fetch_binary, &base_url) {
            ...
            Err(e) => {
                ...
                if msg.contains("fetch failed") {
                    web_sys::console::log_1(
                        &format!("conda-wasm: no shard index for {base}/{subdir}, trying fallback")
                            .into(),
                    );
                } else {
                    web_sys::console::warn_1(
                        &format!("conda-wasm: sharded repodata failed for {base}/{subdir}: {msg}")
                            .into(),
                    );
                }
            }
...
fn parse_repodata_text(
    ...
    let repo: rattler_conda_types::RepoData = serde_json::from_str(text).map_err(|e| {
        CondaWasmError::RepodataParse(format!("parsing repodata for {channel_url}/{subdir}: {e}"))
    })?;
...
pub(crate) fn fetch_sharded_records(
    ...
    while !queue.is_empty() && n_fetched < MAX_SHARDS {
        ...
        for name in &queue {
            ...
            match get_or_fetch_shard(&shard_url, name, fetch_binary, base_url, &channel_url) {
                ...
                Err(e) => {
                    web_sys::console::warn_1(
                        &format!("conda-wasm: failed to fetch shard for {name}: {e}").into(),
                    );
                }
            }
        }
    ...
    web_sys::console::log_1(
        &format!(
            "conda-wasm: {n_fetched} shards ({n_cached} cached, {} fetched) => {} records for {base}/{subdir} [cache: {total_indices} indices, {total_shards} shards]",
            n_fetched - n_cached,
            all_records.len(),
        )
        .into(),
