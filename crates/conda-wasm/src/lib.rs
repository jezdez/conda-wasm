...
use error::CondaWasmError;

#[cfg(conda_wasm_embedded_lockfile)]
const EMBEDDED_LOCKFILE: &str = include_str!(concat!(env!("OUT_DIR"), "/embedded_lockfile.txt"));

#[cfg(conda_wasm_embedded_platform)]
...
fn to_js<T: Serialize>(value: &T) -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(value)
        .map_err(|e| CondaWasmError::SerializeFailed(e.to_string()).into())
}

fn parse_platform(platform_str: &str) -> Result<Platform, JsValue> {
    Platform::from_str(platform_str)
        .map_err(|_| CondaWasmError::PlatformUnknown(platform_str.to_string()).into())
}

#[wasm_bindgen]
pub fn conda_wasm_init() -> String {
    web_sys::console::log_1(&"conda-wasm initialized".into());
    format!("conda-wasm v{}", env!("CARGO_PKG_VERSION"))
}

#[wasm_bindgen]
pub fn conda_wasm_embedded_lockfile() -> Option<String> {
    #[cfg(conda_wasm_embedded_lockfile)]
    {
        Some(EMBEDDED_LOCKFILE.to_string())
    }
    #[cfg(not(conda_wasm_embedded_lockfile))]
    {
        None
    }
}
...
#[wasm_bindgen]
pub fn conda_wasm_embedded_platform() -> Option<String> {
    #[cfg(conda_wasm_embedded_platform)]
    {
        Some(EMBEDDED_PLATFORM.to_string())
    }
    #[cfg(not(conda_wasm_embedded_platform))]
    {
        None
    }
}
...
pub(crate) async fn fetch_bytes(url: &str) -> Result<Vec<u8>, CondaWasmError> {
    ...
    let controller = AbortController::new()
        .map_err(|e| CondaWasmError::FetchFailed(format!("AbortController error: {e:?}")))?;
    ...
    let request = Request::new_with_str_and_init(url, &opts)
        .map_err(|e| CondaWasmError::FetchFailed(format!("request error: {e:?}")))?;
    ...
    let result = async {
        let resp_val = JsFuture::from(global_fetch(&request)).await.map_err(|e| {
            CondaWasmError::FetchFailed(format!("fetch error (timeout or CORS?): {e:?}"))
        })?;
        let resp: Response = resp_val
            .dyn_into()
            .map_err(|_| CondaWasmError::FetchFailed("response cast failed".into()))?;

        if !resp.ok() {
            return Err(CondaWasmError::FetchFailed(format!(
                "HTTP {}",
                resp.status()
            )));
        }
        ...
        let buf_promise = resp
            .array_buffer()
            .map_err(|e| CondaWasmError::FetchFailed(format!("array_buffer error: {e:?}")))?;
        let buf_val = JsFuture::from(buf_promise)
            .await
            .map_err(|e| CondaWasmError::FetchFailed(format!("buffer read error: {e:?}")))?;
        let buf: ArrayBuffer = buf_val
            .dyn_into()
            .map_err(|_| CondaWasmError::FetchFailed("buffer cast failed".into()))?;
...
#[wasm_bindgen]
pub async fn conda_wasm_bootstrap_streaming(
...
#[wasm_bindgen]
pub fn conda_wasm_extract_package(
    bytes: &[u8],
    filename: &str,
    on_file: js_sys::Function,
) -> Result<JsValue, JsValue> {
    let mut file_cb = |path: &str, data: &[u8]| -> Result<(), CondaWasmError> {
        let js_path = JsValue::from(path);
        let js_bytes = js_sys::Uint8Array::from(data);
        on_file
            .call2(&JsValue::NULL, &js_path, &js_bytes)
            .map_err(|e| CondaWasmError::CallbackFailed(format!("{e:?}")))?;
        Ok(())
    };
    ...
    let stats = if filename.ends_with(".conda") {
        ...
        return Err(CondaWasmError::UnknownPackageFormat(filename.to_string()).into());
    };
...
#[wasm_bindgen]
pub fn conda_wasm_bootstrap_plan(
