/**
 * conda-wasm-worker.js — Web Worker for conda in the browser.
 *
 ...
 * side-channel postMessage events tagged with `__conda_wasm: true`.
 *
 * Loaded as a classic Worker; uses importScripts for pyjs runtime and dynamic
 * import() for ES modules (Comlink, conda_wasm).
 */
'use strict';
...
(async () => {
    ...
    const {
        default: init, conda_wasm_init, conda_wasm_bootstrap_streaming, conda_wasm_bootstrap_plan,
        conda_wasm_extract_package, conda_wasm_solve_init, conda_wasm_fetch_and_solve
    } = await import('./conda_wasm.js' + _bustCache);
    ...
    function emit(type, data) {
        self.postMessage({ __conda_wasm: true, type, ...data });
    }
    ...
    var CACHE_DB_NAME = 'conda-wasm-bootstrap';
    ...
    async function doBootstrap(opts) {
        ...
        // Step 1: Load conda-wasm
        log('Loading conda-wasm module...');
        await init();
        var condaWasmVersion = conda_wasm_init();
        var solveVersion = conda_wasm_solve_init();
        log('conda-wasm loaded: ' + condaWasmVersion + ' / ' + solveVersion, 'ok');

        self.conda_wasm_extract_package = conda_wasm_extract_package;
        ...
        // (registered as self._conda_wasm_json_dumps during bootstrap).
        self.fetch_and_solve = function (request) {
            var jsRequest;
            if (typeof request === 'string') {
                jsRequest = JSON.parse(request);
            } else if (self._conda_wasm_json_dumps && typeof request === 'object' && request !== null) {
                var jsonStr = String(self._conda_wasm_json_dumps(request));
                jsRequest = JSON.parse(jsonStr);
            } else {
                jsRequest = request;
            }
            return conda_wasm_fetch_and_solve(
        ...
        var plan = conda_wasm_bootstrap_plan(lockfileText, platform);
        ...
        if (cachedPackages && cachedPackages.length > 0) {
            ...
            var onFile = function (pkgName, path, bytes) {
                try {
                    ...
                    console.error('[conda-wasm-worker] onFile error:', pkgName, path, e);
                    throw e;
                }
            };
            ...
            result = await conda_wasm_bootstrap_streaming(lockfileText, platform, streamProgress, onFile);
        ...
        try {
            pyjsModule['exec'](
                'import json, js\n' +
                'js._conda_wasm_json_dumps = json.dumps\n'
            );
        } catch (e) {
    ...
    self.postMessage({ __conda_wasm: true, type: 'ready' });
})();
