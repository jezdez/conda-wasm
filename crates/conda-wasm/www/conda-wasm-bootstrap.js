/**
 * conda-wasm-bootstrap.js — Thin Comlink client for the conda-wasm worker.
 *
 ...
 *
 * Usage:
 *   import { createCondaWorker } from './conda-wasm-bootstrap.js';
 ...
 *   });
 *   const result = await conda.bootstrap({ lockfileUrl: '/conda-wasm.lock' });
...
export async function createCondaWorker({ onLog, onProgress, onPrint, onError } = {}) {
    const workerUrl = new URL('./conda-wasm-worker.js?v=' + Date.now(), import.meta.url);
    ...
    await new Promise((resolve) => {
        const onReady = (e) => {
            if (e.data?.__conda_wasm && e.data.type === 'ready') {
